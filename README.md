...
详细接口文档（含请求/响应格式、参数说明、数据库表结构）请参阅 [API.md](./docs/API.md)。
...
项目配置了 GitHub Actions 自动构建验证（`.github/workflows/ci.yml`）：

- **前端构建**：每次 push 到 main 分支自动执行 `npm ci` + `next build`
- **后端检查**：Python 语法验证 + Flask 依赖安装测试
